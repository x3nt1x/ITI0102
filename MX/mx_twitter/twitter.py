...
def find_fastest_growing(tweets: list) -> Tweet:
    ...
    """
    fastest_growing = min(tweet.time / tweet.retweets if tweet.time > 0 else 0 for tweet in tweets)
    return [tweet for tweet in tweets if (tweet.time / tweet.retweets if tweet.time > 0 else 0) == fastest_growing][0]
...
def sort_hashtags_by_popularity(tweets: list) -> list:
    ...
    for tweet in tweets:
        for match in re.finditer(r"#\w+", tweet.content):
            hashtag = match.group()

            if hashtag in hashtags:
                hashtags[hashtag] += tweet.retweets
            else:
                hashtags[hashtag] = tweet.retweets

    return [tweet[0] for tweet in sorted(hashtags.items(), key=lambda x: (-x[1], x[0]))]
